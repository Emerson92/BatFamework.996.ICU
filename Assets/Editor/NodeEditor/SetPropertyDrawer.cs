...
public class SetPropertyDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
	{
		...
		{
            OriginalType = property.serializedObject.targetObject;
			...
			if (pi == null)
			{
                try
                {
                    Type originalType = property.serializedObject.targetObject.GetType();
                    PropertyInfo info = originalType.GetProperty(setProperty.Name);
                }
                catch {
